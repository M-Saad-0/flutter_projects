...
import 'package:perfume_store_admin/models/order.dart';
...
class OrderProvider extends ChangeNotifier {
  ...
  Future<void> _loadorderItems() async {
    ...
    notifyListeners();
    try {
      QuerySnapshot query = await _firestore.collection('orders').get();
      _order = query.docs
  ...
  Future<void> deleteOrderItem(MyOrder order) async {
    try {
      ...
      _order.removeWhere((e) => e.id == order.id);
      notifyListeners();
    } catch (e) {
  ...
  Future<void> placeOrder(MyOrder order) async {
    try {
      ...
      if (ordersToPlace.docs.isNotEmpty) {
        ...
        _order = _order
            .map((e) => e.id == order.id ? e.copyWith(placed: true) : e)
            .toList();
        notifyListeners();
      }
    } catch (e) {
